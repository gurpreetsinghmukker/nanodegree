...
// find which regions the ball is in
    ...
    if( 0 <= ball_position < left_region_boundary ) // left region
    {
        // turn to the left
        ROS_INFO("turning left");
        drive_robot(0.0, 0.5);
    }
    else if( left_region_boundary <= ball_position < mid_region_boundary ) // mid region
    {
        ROS_INFO("driving straight");
        drive_robot(0.1,0.0);
    }
    else if(  mid_region_boundary <= ball_position <= img.width )// right region
    {
        // turn to the right
